...
const viewHtmlHandler = () => {
    return gulp.src([path.join(__dirname, 'src', "app", "**", '*.html')])
        .pipe(htmlmin({
            collapseWhitespace: true,
            removeEmptyAttributes: true,
            collapseBooleanAttributes: true,
...
const watchHandler = () => {
    ...
    gulp.watch("./src/app/**/*.html", viewHtmlHandler)
    gulp.watch("./src/app/**/*.ts", tsHandler);
}

const devStart = gulp.series(
    delHandler,
    gulp.parallel(sassHandler, tsHandler, htmlHandler, viewHtmlHandler, libHandler,imgHandler),
    serverHandler,
    watchHandler
)
